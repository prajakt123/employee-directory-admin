define({ 
  ...
  addDepartmentTofilter:function(departmentList){
    ...
    //     this.view.segDepartment.removeAll();
    //     this.view.segDepartment.addAll(deptList);
    debugger;
    ...
    if(Array.isArray(employeeList)){
      ...
      for(var i=0;i<employeeLength;i++){
        ...
        if(empObj["empimage"]===undefined){
          segObj["empimage"]={"src":"empdefaultimageicon.png"};
        }else{
          segObj["empimage"]=empObj["empimage"];
        }
        if(isAdmin(this.userEmail)){
          segObj["imgEdit"]={"isVisible":true,"src":"empediticon.png"};
          segObj["imgRemove"]={"isVisible":true,"src":"empremoveicon.png"};
        }else{
          segObj["imgEdit"]={"isVisible":false,"src":"empediticon.png"};
          segObj["imgRemove"]={"isVisible":false,"src":"empremoveicon.png"};
        }
        segObj["flxDepAndId"]={"isVisible":false};
        segObj["flxDepIdDesig"]={"isVisible":false};
        segObj["imgDropDown"]={"src":"drop_down.png"};
  ...
  getImageByMediaId : function(mediaResp, curIndex, TotCount,finalCallback){
    ...
    objSvc.getBinaryContent({"dataObject":dataObject, "binaryAttrName": "Image"},getBinaryContentSuccessCallback,getBinaryContentFailureCallback);
  ...
  doneCompletingBinaryDownloads: function(){
    ...
    for (var i = 0; i<this.parsedEmployeeList.length;i++){          
      ...
      if (profilePicMap[this.parsedEmployeeList[i].Employee_id][0].Url !== undefined && profilePicMap[this.parsedEmployeeList[i].Employee_id][0].Url !== null && profilePicMap[this.parsedEmployeeList[i].Employee_id][0].Url.length >0){
        this.parsedEmployeeList[i].empimage = {src:profilePicMap[this.parsedEmployeeList[i].Employee_id][0].Url}; 
      }
    ...
    this.populateDataToSegment(this.parsedEmployeeList);
    this.view.Group.setVisibility(true);
    this.view.forceLayout();
    return;
  },
  ...
  removeBinary:function(mediaIdParam){
    ...
    objSvc.deleteBinary(
      ...
      function(err){kony.print("Failed to remove Binary: " + JSON.stringify(err));}    
    );
  },
  ...
  removeMediaRecord: function(mediaIdParam){
    ...
    objSvc.deleteRecord( {"dataObject":mediaObject},
  ...
  removeContactObject:function(contactIdParam){
    ...
    objSvc.deleteRecord( {"dataObject":dataObject},
  ...
  processSearch:function (searchstring){
    ...
    if (searchstring.length === 0){
      ...
      for (var i=0;i<this.employeeList.length;i++){
        ...
        }else if((this.employeeList[i].Employee_id.toLowerCase().indexOf(searchstring) != -1)){
          segData.push(this.employeeList[i]);
        }else if(this.employeeList[i].designation[0]!==undefined){
        ...
        }else if(this.employeeList[i].department[0]!==undefined){
    ...
    if(context!==null && context!==undefined){
      try{
        ...
        if( data.flxDepAndId.isVisible===true){
          ...
          data.flxDepAndId={"isVisible":false};
        }else{
          data.flxDepAndId={"isVisible":true};
  ...
  expandEmployeeDetailMob:function(context){
    ...
    //     if(this.view.imgDropDown=="drop_down.png"){
    ...
    if(context!==null && context!==undefined){
      try{
        ...
        if( data.flxDepIdDesig.isVisible===true){
          data.imgDropDown.src="drop_down.png";
          data.flxDepIdDesig={"isVisible":false};
        }else{
          data.flxDepIdDesig={"isVisible":true};
          data.imgDropDown.src="drop_down_arrow_orange.png";
        }
  ...
  //   onRowClickTabMode:function(){

  //     var data=this.view.segEmployeeList.selectedRowItems[0];
  //     if( data.flxDepAndId.isVisible===true){
  //       data.flxDepAndId={"isVisible":false};
  //     }else{
  //       data.flxDepAndId={"isVisible":true};
  ...
  //     this.view.segEmployeeList.setDataAt(data, this.view.segEmployeeList.selectedRowIndex[1]);
  ...
  applyFilter:function(){
    ...
    this.view.empHeader1.imgSearch.setVisibility(false);
    this.view.forceLayout();
  ...
  setUIforTablet:function(viewMode){
    debugger;
  ...
  setUIforMobile:function(viewMode){
    debugger;
    this.view.empHeader1.flxHamburger.setVisibility(true);
  ...
  //   showTransparentSkin:function(){
  //     this.view.flxRoot.skin="sknTransparent";
  //   }
